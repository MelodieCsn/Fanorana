...
//			}
//			else 
			if (pionOpposePresent(val1x+deltax,val1y+deltay,p,T)){
...
public static void main(String[] args){//test pour aspirer, on deplace un pion noir puis on le fait avancer de façon à aspirer les pions blancs sur la colonne (4)
	...
	Terr.initPlateau(Terr.T);
	{
		...
		Terr.deplacer(4,2,Terr.T.getPion(4,3));
		System.out.println("avancement du pion et aspiration...");
		Terr.avancer(4, 3, Terr.T.getPion(4,2), Terr.T);
		System.out.println("Pion aux anciennes coordonnées ?  " +Terr.pionPresent(4,2,Terr.T));
		System.out.println("Pion aux nouvelles coordonnées ?  " +Terr.pionPresent(4,3,Terr.T));
		System.out.println("Pion de couleur opposée sur la ligne percutée ou aspirée ?  " +Terr.pionPresent(4,0,Terr.T));
	}
}
